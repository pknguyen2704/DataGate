...
JOB_NAME = "batch_profiling_metric_collection"
...
def add_analyzers_for_column(runner, column_name, data_type):
    runner = (
        runner.addAnalyzer(Completeness(column_name))
    ...
    if isinstance(data_type, NUMERIC_TYPES):
        runner = (
            runner.addAnalyzer(Minimum(column_name))
    ...
    if isinstance(data_type, StringType):
        runner = runner.addAnalyzer(MinLength(column_name)).addAnalyzer(
            MaxLength(column_name)
        )
        ...
        rows.append(
            {
                ...
                "standard_deviation": metric_map.get(
                ...
                "approx_count_distinct": metric_map.get(
                    (column_name, "approx_count_distinct")
                ),
...
def main():
    ...
    try:
        ...
        for table_info in active_tables:
            collect_one_table_profiling(spark,pg_hook,table_info,processing_date_hour)
