...
def parse_args():
    ...
    parser.add_argument("--lgbm-use-barrier-execution-mode", type=parse_bool, default=True)
    parser.add_argument("--lgbm-num-tasks", type=int, default=4)
    parser.add_argument("--lgbm-num-threads", type=int, default=3)
    parser.add_argument("--lgbm-timeout", type=int, default=1200)
    parser.add_argument("--lgbm-use-single-dataset-mode", type=parse_bool, default=True)
    parser.add_argument("--lgbm-data-transfer-mode", default="streaming")
    parser.add_argument("--lgbm-verbosity", type=int, default=-1)
    parser.add_argument("--spark-cleanup-timeout-seconds", type=int, default=30)
    parser.add_argument("--py4j-cleanup-timeout-seconds", type=int, default=5)
    parser.add_argument("--db-cleanup-timeout-seconds", type=int, default=10)
...
def build_runtime_config(args):
    return {
        ...
        "db_cleanup_timeout_seconds": args.db_cleanup_timeout_seconds,
    }
...
def get_anomaly_config(pg_hook, table_id):
    ...
    cfg = {
        ...
        "exclude_cols": [
            validate_identifier(col, "exclude_col") for col in as_list(row[9])
        ],
        "categorical_cols": [
            validate_identifier(col, "categorical_col") for col in as_list(row[10])
        ],
        "numeric_cols": [
            validate_identifier(col, "numeric_col") for col in as_list(row[11])
        ],
    }
...
def get_auc_threshold(pg_hook, parameter_id):
    row = pg_hook.get_first(
        ...
        FROM auc_manual_thresholds
        WHERE model_parameter_id = %s
          AND is_active = TRUE
        ORDER BY
            CASE severity_level
...
def create_spark_session(connection_config):
    ...
    return (
        ...
        .config(
            f"spark.sql.catalog.{catalog}.uri", connection_config["iceberg_rest_url"]
        )
        .config(
            f"spark.sql.catalog.{catalog}.warehouse",
            connection_config["iceberg_warehouse"],
        )
        .config(
...
def save_shap(pg_hook, result_id, processing_hour, rows):
    ...
    sql = """
        ...
        VALUES %s
        ON CONFLICT (auc_result_id, feature_name)
        DO UPDATE SET
...
def save_auc_verify(
    pg_hook, result_id, threshold, auc_score, p_value, processing_hour, anomaly_cfg
):
    if threshold:
...
def run_table(spark, pg_hook, table, processing_hour, runtime_cfg):
    logger.info(
...
def close_pg_hook(pg_hook, timeout_seconds):
    if pg_hook is None:
        return
    ...
    run_with_timeout("postgres-close", timeout_seconds, close_conn)
...
def main():
    ...
    try:
        ...
        close_pg_hook(pg_hook, runtime_cfg["db_cleanup_timeout_seconds"])
        gc.collect()
