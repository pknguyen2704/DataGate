# metadata_collection_task = PythonOperator(
#     task_id="collect_bronze_tables_metadata",
#     python_callable=collect_metadata,
#     op_kwargs={
#         "trino_conn_id": "trino_default",
#         "datagate_db_conn_id": "datagate_db_default",
#         "connection_name": "iceberg",
#         "schema_name": "bronze",
#         "processing_date_hour": "{{ ds }} 12:00:00",
#     },
# )
import logging
import uuid
...
def get_connection_config(pg_hook, connection_name):
    """
    Lấy thông tin connection từ bảng connections.

    DAG truyền vào connection_name.
    Job dùng connection_name để lấy iceberg_catalog_name.
    """
...
def get_active_tables(pg_hook, catalog_name, schema_name):
    """
    Lấy danh sách bảng active từ bảng tables.

    Điều kiện:
    - catalog_name = iceberg_catalog_name lấy từ connection
    - schema_name = schema_name DAG truyền vào
    - is_active = TRUE
    """
...
def insert_table_batch_metadata(pg_hook, table_id, metadata):
    pg_hook.run(
        """
...
def collect_one_table_metadata(
    ...
    logger.info(
        "Saved metadata | table=%s.%s.%s | table_id=%s | processing_date_hour=%s",
        catalog_name,
        schema_name,
        table_name,
        table_id,
