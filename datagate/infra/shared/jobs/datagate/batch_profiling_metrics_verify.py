...
def build_profiling_results(rows, processing_date_hour):
    results = []
    for row in rows:
        threshold_id, batch_id, metric_name, min_th, max_th, severity, completeness, mean, std, min_v, max_v, min_len, max_len, distinctness, approx_distinct = row
        values = {
...
def evaluate_profiling_metrics(datagate_db_conn_id, connection_name, schema_name, processing_date_hour):
