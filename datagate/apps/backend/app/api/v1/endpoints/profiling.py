...
@router.get("/runs")
def get_profiling_runs(db: Session = Depends(get_db)):
...
def get_profiling_run_detail(run_id: int, db: Session = Depends(get_db)):
...
def get_column_histogram(col_profile_id: int, db: Session = Depends(get_db)):
...
@router.get("/trend")
def get_profiling_trend(table: str, column: str, metric: str = "completeness", db: Session = Depends(get_db)):
