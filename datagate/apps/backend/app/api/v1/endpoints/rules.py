...
def get_rules(table: str = None, db: Session = Depends(get_db)):
...
def create_rule(rule: ActiveRuleCreate, db: Session = Depends(get_db)):
...
def update_rule(rule_id: int, rule_update: ActiveRuleUpdate, db: Session = Depends(get_db)):
