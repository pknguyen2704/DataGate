...
from sqlalchemy import BigInteger, Column, DateTime, Float, ForeignKey, Index, Integer, String, func, Enum, Boolean, Text
...
class BatchTableProfiling(Base):
    ...
    batch_table_profiling_metrics_verify = relationship("BatchTableProfilingMetricsVerify", back_populates="batch_table_profiling", cascade="all, delete-orphan")

    __table_args__ = (
        Index("ix_batch_table_profiling__table_hour_column", "table_id", "processing_date_hour", "column_name", unique=True),
        Index("ix_batch_table_profiling__table_column_hour", "table_id", "column_name", "processing_date_hour"),
...
class BatchTableProfilingManualThreshold(Base):
    ...
    table = relationship("Table", back_populates="batch_table_profiling_manual_thresholds")
    results = relationship("BatchTableProfilingMetricsVerify", back_populates="profiling_manual_threshold", cascade="all, delete-orphan")
    created_by_user = relationship("User", foreign_keys=[created_by])
    last_modified_by_user = relationship("User", foreign_keys=[last_modified_by])

    __table_args__ = (
        Index("ix_batch_table_profiling_manual_thresholds__table_column_metric", "table_id", "column_name", "metric_name", unique=True),
        Index("ix_batch_table_profiling_manual_thresholds__table_active", "table_id", "is_active"),
        Index("ix_batch_table_profiling_manual_thresholds__lookup", "table_id", "column_name", "metric_name", "is_active"),
    )
...
class BatchTableProfilingMetricsVerify(Base):
    ...
    profiling_manual_threshold_id = Column(UUID(as_uuid=False), ForeignKey("batch_table_profiling_manual_thresholds.id", ondelete="CASCADE"), nullable=False)
    batch_table_profiling_id = Column(UUID(as_uuid=False), ForeignKey("batch_table_profiling.id", ondelete="CASCADE"), nullable=False)
    ...
    __table_args__ = (
        ...
        Index("ix_batch_table_profiling_metrics_verify__threshold_batch", "profiling_manual_threshold_id", "batch_table_profiling_id"),
        Index("ix_batch_table_profiling_metrics_verify__batch_status", "batch_table_profiling_id", "status"),
    )
