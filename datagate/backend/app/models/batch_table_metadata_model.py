...
class BatchTableMetadata(Base):
    ...
    batch_table_metadata_metrics_verify = relationship(
...
class BatchTableMetadataManualThreshold(Base):
    ...
    table = relationship(
        "Table", back_populates="batch_table_metadata_manual_thresholds"
    )
    results = relationship(
        "BatchTableMetadataMetricsVerify",
        back_populates="metadata_manual_threshold",
        cascade="all, delete-orphan",
    )
    ...
    __table_args__ = (
        ...
        Index(
            "ix_batch_table_metadata_manual_thresholds__table_active",
            "table_id",
            "is_active",
        ),
        Index(
            "ix_batch_table_metadata_manual_thresholds__lookup",
            "table_id",
            "metric_name",
            "is_active",
        ),
...
class BatchTableMetadataMetricsVerify(Base):
    ...
    metadata_manual_threshold_id = Column(
        UUID(as_uuid=False),
        ForeignKey("batch_table_metadata_manual_thresholds.id", ondelete="CASCADE"),
        nullable=False,
    )
    batch_table_metadata_id = Column(
        UUID(as_uuid=False),
        ForeignKey("batch_table_metadata.id", ondelete="CASCADE"),
        nullable=False,
    )
    ...
    metadata_manual_threshold = relationship(
        "BatchTableMetadataManualThreshold", back_populates="results"
    )
    batch_table_metadata = relationship(
        "BatchTableMetadata", back_populates="batch_table_metadata_metrics_verify"
    )
    ...
    __table_args__ = (
        ...
        Index(
            "ix_batch_table_metadata_metrics_verify__threshold_batch",
            "metadata_manual_threshold_id",
            "batch_table_metadata_id",
        ),
        Index(
            "ix_batch_table_metadata_metrics_verify__batch_status",
            "batch_table_metadata_id",
            "status",
        ),
    )
