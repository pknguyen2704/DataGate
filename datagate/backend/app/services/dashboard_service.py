...
class DashboardService:
    ...
    def table_healths(self, hour: datetime | None = None) -> list[dict]:
        tables = self.db.query(Table).filter(Table.is_active.is_(True)).order_by(Table.schema_name, Table.table_name).all()
    ...
    def get_platform_overview(self, schema_name: str | None = None) -> dict:
        query = self.db.query(Table).filter(Table.is_active.is_(True))
        if schema_name:
    ...
    def get_timeline_stats(self, schema_name: str | None = None) -> list[dict]:
        hours = self.processing_hours()
        timeline = []
        checks = [
    ...
    def managed_tree(self) -> list[dict]:
        rows = self.db.query(Table).filter(Table.is_active.is_(True)).order_by(Table.schema_name, Table.table_name).all()
        grouped = {}
        for table in rows:
    ...
    def grafana_variables(self) -> dict:
        rows = self.db.query(Table).filter(Table.is_active.is_(True)).all()
        return {"schemas": sorted({row.schema_name for row in rows}), "tables": sorted({row.table_name for row in rows}), "processing_date_hours": self.processing_hours()}
