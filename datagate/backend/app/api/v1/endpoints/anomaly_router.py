...
def verify_results(db: Session = Depends(get_db), _user: User = Depends(require_permission(PermissionCode.ANOMALY_VIEW))):
...
def resolve_verify_result(verify_id: UUID, db: Session = Depends(get_db), _user: User = Depends(require_permission(PermissionCode.ANOMALY_RUN))):
...
def unresolve_verify_result(verify_id: UUID, db: Session = Depends(get_db), _user: User = Depends(require_permission(PermissionCode.ANOMALY_RUN))):
    row = db.query(LightGBMAUCVerify).filter(LightGBMAUCVerify.id == str(verify_id)).first()
