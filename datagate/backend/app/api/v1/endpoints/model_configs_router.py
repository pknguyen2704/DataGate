...
from app.schemas.lightgbm_schema import (
    ...
    LightGBMAnomalyConfigCreate, LightGBMAnomalyConfigListOut,
    LightGBMAnomalyConfigOut, LightGBMAnomalyConfigUpdate,
)
...
def upload_json(
    ...
    data: dict = Body(...),
    service: ModelConfigService = Depends(get_model_config_service),
    current_user: User = Depends(require_permission(PermissionCode.MODEL_CONFIG_UPDATE))
):
    return service.upload_json(str(table_id), data, str(current_user.id))


@model_configs_router.get("/configs", response_model=LightGBMAnomalyConfigListOut)
...
@model_configs_router.post("/configs/upload-json", response_model=LightGBMAnomalyConfigOut)
...
@model_configs_router.get("/configs/{config_id}", response_model=LightGBMAnomalyConfigOut)
...
@model_configs_router.post("/configs", response_model=LightGBMAnomalyConfigOut, status_code=status.HTTP_201_CREATED)
...
@model_configs_router.patch("/configs/{config_id}", response_model=LightGBMAnomalyConfigOut)
...
@model_configs_router.delete("/configs/{config_id}", status_code=status.HTTP_204_NO_CONTENT)
