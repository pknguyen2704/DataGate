...
SIM_START = "2025-01-01 12:00:00"
SIM_END = "2025-02-01 00:00:00"
SIM_STEP_HOURS = 12
SIM_VAR_NAME = "citi_bike_tripdata_next_processing_date_hour"
...
task_fail_slack_alert = send_slack_webhook_notification(
    ...
    text="\n".join([
        ...
        "*Try*: `{{ ti.try_number }}`",
        "*Execution Time*: `{{ ts }}`",
        "*Processing Date Hour*: `{{ ti.xcom_pull(task_ids='get_processing_date_hour') }}`",
        "",
...
def get_processing_date_hour():
    ...
    if current_dt > datetime.fromisoformat(SIM_END):
        raise AirflowSkipException(f"Simulation completed. current={value}, end={SIM_END}")
...
with DAG(
    dag_id="citi_bike_tripdata_pipeline_experiment_version",
    ...
    tags=["datagate", "iceberg", "citi_bike_tripdata", "simulation"],
) as dag:
    ...
    [gold_meta_verify, gold_profile_verify] >> gold_rule_verification >> gold_gate >> rule_suggestion >> advance_processing_date_hour_task
