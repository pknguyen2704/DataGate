...
DEFAULT_OUTPUT_DIR = Path("experiments/data/citi_bike_tripdata")
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Tạo CSV dirty từ CSV gốc Citi Bike tripdata."
    )

    source = parser.add_mutually_exclusive_group(required=True)
    source.add_argument("--file", help="Đường dẫn CSV gốc.")
    source.add_argument("--data-dir", help="Thư mục chứa các file CSV gốc.")

    parser.add_argument(
        "--output-dir",
        default=str(DEFAULT_OUTPUT_DIR),
        help="Thư mục ghi CSV dirty khi xử lý một hoặc nhiều file.",
    )
    parser.add_argument(
        "--output-file",
        help="File CSV dirty đầu ra. Chỉ dùng khi truyền --file.",
    )
    parser.add_argument(
        "--suffix",
        default="_dirty",
...
def inject_dirty_data_pandas(
    ...
    idx = selected(0)
    if len(idx) > 0:
        df.loc[idx, ["start_lat", "start_lng"]] = ["0.0", "0.0"]

    idx = selected(1)
    if len(idx) > 0:
        df.loc[idx, ["end_lat", "end_lng"]] = ["0.0", "0.0"]

    idx = selected(2)
    if len(idx) > 0:
        df.loc[idx, "member_casual"] = "unknown"

    idx = selected(3)
    if len(idx) > 0:
        df.loc[idx, "rideable_type"] = "invalid_bike"

    idx = selected(4)
    if len(idx) > 0:
        df.loc[idx, ["start_station_id", "end_station_id"]] = "INVALID_STATION"
        df.loc[idx, ["start_station_name", "end_station_name"]] = "INVALID_STATION"
...
def check_output_path(input_file: Path, output_file: Path, overwrite: bool) -> Path:
...
def process_file(input_file: Path, output_file: Path, args) -> Tuple[int, int]:
    temp_output_file = check_output_path(input_file, output_file, args.overwrite)
    ...
    with tqdm(total=count_data_rows(input_file)) as pbar:
        for df in pd.read_csv(input_file, chunksize=args.chunksize, dtype=str):
            raw_len = len(df)
            pbar.update(raw_len)
            ...
            if any(col not in df.columns for col in TIME_COLUMNS):
                raise ValueError(f"CSV thiếu một trong các cột thời gian: {sorted(TIME_COLUMNS)}")
            ...
            df.loc[:, original_columns].to_csv(
                temp_output_file,
