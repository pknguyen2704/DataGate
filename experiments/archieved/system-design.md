...
| Quản lý kết nối nền tảng dữ liệu  |                                 Xem thông tin kết nối |   Có  |       Có      |     Không    | Data Engineer được xem phục vụ vận hành; Data Analyst không được truy cập.             |
...
| Quản lý bảng tích hợp            |                               Xem danh sách bảng       |   Có  |       Có      |     Không    | Gộp chung cùng với xem connection, Analyst không được truy cập.                        |
| Quản lý bảng tích hợp            |        Thêm mới, Bật/tắt theo dõi bảng, Xóa bảng      |   Có  |     Không     |     Không    | Chỉ Admin mới có quyền quản lý/thay đổi trạng thái tích hợp của bảng.                  |
| Quản lý tham số mô hình           |                                   Xem tham số mô hình |   Có  |       Có      |      Có      | Các vai trò có thể xem để hiểu cấu hình phát hiện bất thường.                           |
| Quản lý tham số mô hình           |                  Thêm, sửa, xóa thông tin tham số     |   Có  |       Có      |     Không    | Cả Admin và Engineer đều được phân quyền đầy đủ như nhau (kể cả xóa).                  |
...
| Quản lý ngưỡng cảnh báo           |                        Thêm, sửa, xóa ngưỡng cảnh báo |   Có  |       Có      |     Không    | Cả Admin và Engineer đều được phân quyền đầy đủ như nhau (kể cả xóa).                  |
| Quản lý luật dữ liệu              |                           Xem, thêm mới, sửa luật dữ liệu |   Có  |       Có      |      Có      | Analyst có toàn quyền xem, tạo mới và chỉnh sửa luật (bỏ phần đề xuất nháp/suggest).     |
| Quản lý luật dữ liệu              |                Phê duyệt, bật/tắt và xóa luật dữ liệu |   Có  |     Không     |     Không    | Chỉ Admin được phép phê duyệt, bật/tắt hoạt động hoặc xóa luật dữ liệu.                 |
...
| Quản lý chất lượng dữ liệu        |  Cập nhật trạng thái xử lý kết quả chất lượng dữ liệu |   Có  |     Không     |     Không    | Chỉ Admin có quyền cập nhật trạng thái resolve cảnh báo.                                |
